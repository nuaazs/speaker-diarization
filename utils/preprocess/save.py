...
def save_file(file, spk, channel, upload=False):
    ...
    save_path_wav = os.path.join(spk_dir, f"raw_{speech_number}.bin")
    logger.info("\t\tSave file path: {save_path}")
    file.save(save_path)
    # conver to wav
    logger.info("\t\tConver to wav.")
    end = cfg.WAV_START + cfg.WAV_LENGTH
    cmd = f"ffmpeg -i {save_path} -y -ss {cfg.WAV_START} -to {end} -ar {cfg.SR} -ac 1 -f s16le -acodec pcm_s16le -vn -map_metadata -1 -y  {save_path_wav} > /dev/null 2>&1"
    # print(cmd)
    run_cmd(cmd)
...
def save_url(url, spk, channel, upload=False):
    ...
    run_cmd(f"ffmpeg -i {save_path} -y -ss {cfg.WAV_START} -to {end} -ar {cfg.SR} -ac 1 -f s16le -acodec pcm_s16le -vn -map_metadata -1 -y  {save_path_wav} > /dev/null 2>&1")
